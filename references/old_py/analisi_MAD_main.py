# -*- coding: utf-8 -*-
"""
Created on Fri Jun 28 16:40:54 2024
...
warnings.filterwarnings('ignore')
import sys
sys.path.insert(0, 'P:/TC_BU/W-PY_DATA/data_SAP/BIN/utils')
from Timer_lib import Timer

min_date="2023-01-01"
plant_in="(1,3,4)" #vcp, ccp,cfp
status_samord_min=6 #da attesa produzione in poi
status_cusreq_min=4 #da attesa valutazione in poi
days_on_time=2 #numero di giorni <=per cui ttm e otd sono on time
SAVE_FOLDER='P:/TC_BU/W-PY_DATA/rpt/mag/data_mad/'
file_SamOrd=SAVE_FOLDER+'MADSamOrd.xlsx'
file_CusReq=SAVE_FOLDER+'MADCusReq.xlsx'


def open_MyConn():
    try:
        mycon=mysql.connector.connect(host='172.31.1.53',user='madaccess',password='materials',database='cm_db',charset='utf8')
        ...
        print('Error while connecting to MySlq',e)
    finally:
        pass  
    return mycon
def close_MyConn(mycon):
    if mycon.is_connected():
        cursor=mycon.cursor()
        cursor.close()
        mycon.close()
        print('mySql connecction is closed')
    else:
        print('mySql connecction is already closed')

def RetriveSamOrd(mycon,days_on_time):
    query=f"""SELECT
    ...
    #AND Plant.Plant='VCP''SamOrd'
    Timer.Display('Import SamOrd','[..]',cr=0)
   
    df=(pd.read_sql(query,mycon)
#        .assign(otd_delay_days = lambda df_: (df_.DateEndPrd-df_.DateDlvPlanned).dt.days) 
    ...
    return df.sort_values(by=['DateIns'],ascending=[True])

def RetriveCusReq(mycon):
    query=f"""SELECT 
...
def SaveFile(df, fileName):
    Timer.Display(f'Salvataggio {fileName}','[..]',cr=0)
    df.to_excel(fileName, index=False)
    Timer.Display(f'Salvataggio {fileName}','OK')
...
def main():
    ...
    mycon=open_MyConn()
    df_samord=RetriveSamOrd(mycon,days_on_time)
    df_cusreq=RetriveCusReq(mycon)
    close_MyConn(mycon)
    t.stop()
    t=Timer('Salvataggio')
    t.start()
    SaveFile(df_samord,file_SamOrd)
    SaveFile(df_cusreq,file_CusReq)
    t.stop()
